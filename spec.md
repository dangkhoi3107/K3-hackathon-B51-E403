...
- **Mức prototype nhắm tới:** [x] Working (`codebase/src/index.html` + `codebase/src/app.js` phía frontend, gọi backend cục bộ `codebase/server/` (FastAPI) — backend giữ API key và gọi thật Gemini/OpenRouter, đồng thời có vector search + log tương tác trong SQLite).
...
- **Kết quả lượt deterministic offline (31/07/2026):** **20/20 case đạt (100%)** — không gọi AI thật, chỉ kiểm tra template/rule-based (retrieval, citation guard, fallback, quiz grounding, weighted rubric, input guardrail) trên nội dung tĩnh; chi tiết ở `eval/eval_results.json`. Kết quả này **không thay thế** lượt đánh giá model live.
- **Kết quả lượt live qua Gemini thật (31/07/2026, `eval/run_golden_set_live.mjs` — gọi thật `/api/llm/generate` qua backend đang chạy):** **15/17 case đạt (88,2%)** trên các case có gọi AI (3 case guardrail/validator không gọi AI theo đúng thiết kế, không tính vào tỷ lệ); đạt quality bar $\ge 85\%$. Chi tiết ở `eval/eval_results_live.json`.
  - **1 failure đáng kể nhất (TC05 — lặp lại ở cả 2 lượt chạy):** Model trả lời đúng nội dung Five Whys, có trích dẫn `[Trang 4]` hợp lệ, nhưng diễn giải nguyên nhân gốc bằng cụm *"nguyên nhân gốc rễ"/"root cause"* thay vì đúng cụm từ khoá golden set yêu cầu *"nguyên nhân thật"* — nội dung đúng nhưng cách chấm bằng khớp cụm từ chính xác (substring) quá cứng nhắc trước cách diễn đạt tự nhiên của model thật. Không phải lỗi grounding/bịa nội dung.
  - **Ghi chú phương pháp:** live eval có dao động giữa các lượt chạy (LLM không tất định) — ví dụ TC04 đạt ở lượt 1 nhưng trượt ở lượt 2 do model bỏ sót ví dụ cụ thể (Jira/Slack) dù vẫn đúng trích dẫn trang. Nên chạy lại `run_golden_set_live.mjs` vài lượt trước CP6 và lấy số ổn định nhất cho demo, không chỉ dựa 1 lượt duy nhất.

---

## §8. Phân Công & Kế Hoạch
- **Phân công** (đối chiếu `git log --stat` 30/07 — chỉ Khôi và Hiển có commit tính đến thời điểm này):
  - **Phạm Nguyễn Đăng Khôi (Leader):** Quiz-agent thật tích hợp trong `codebase/src/app.js` (agent 6-tool, quiz thích ứng, quiz theo từng trang, quiz nhắm điểm yếu theo lịch sử tương tác) + Q&A vùng ảnh (vision region) và ảnh gửi trong chat trong `codebase/src/app.js`/`codebase/src/grounding.mjs` + backend `codebase/server/` (vector DB, log) + điều phối chung, review chéo trước mỗi mốc. *(Sketch Python ban đầu ở `codebase/quiz-agent/` đã được thay hoàn toàn bởi bản Working này — xem Changelog §9.)*
  - **Vi Minh Hiển:** đã viết `spec.md`, engine gốc (`codebase/src/grounding.mjs`, `codebase/src/app.js`, `codebase/src/data.js`, `codebase/src/prompts.js`), khung `eval/`; tiếp tục phụ trách UI (`codebase/src/index.html`, `codebase/src/styles.css`) và API/model live (`codebase/src/providers.mjs`).
  ...
  - **Đỗ Tuấn Sơn:** Kiểm thử — đã bổ sung 10/24 case golden set gắn mã turn thật từ chatlog (TC01, TC02, TC03, TC04, TC06, TC10, TC21-24; xem `source_turn_id` trong `eval/golden_set.json`), offline 24/24 pass; còn cần chạy lại lượt live qua Gemini với bộ 24 case mới trước CP6 (`eval/run_golden_set_live.mjs` → `eval/eval_results_live.json`, hiện vẫn là số 15/17 của bộ 20 case cũ).
  - **Trần Đức Bảo Trung (2A202601269):** Canvas & Slide Deck — viết `VLearn_Practice_Coach_Canvas.md` và slide kiến trúc/agent `docs/agent-architecture-deck.html` (deploy trên Claude, thay cho `demo-slides.pdf`), dry run trước CP5/CP6, chuẩn bị case lỗi live cho Q&A.
...
| 30/07/2026 17:45 | Tách nhãn badge/màu sắc cho "Kiến thức nền ngoài slide", cải tiến verifier tự động gỡ citation sai để giữ câu trả lời hữu ích, thêm timer 3 phút cho Quiz UI | Cải tiến dựa trên phản hồi từ vòng User Validation với người dùng ngoài nhóm (Validation Feedback Log #1, #3, #5) |
...
| 30/07/2026 | Thêm backend cục bộ (`codebase/server/`, FastAPI): giữ API key thay vì học viên tự dán; vector search server-side có cache SQLite (`slide_vectors`) thay localStorage-mỗi-trình-duyệt; log tương tác/quiz thật (`interaction_log`, `quiz_results`) xuất qua `/api/logs/export` phục vụ `eval/`/`validation/` | Học viên test không cần API key riêng; vector dùng chung giữa mọi người thay vì tính lại mỗi trình duyệt; cần dữ liệu thật để bù case còn thiếu trong golden set và validation |
| 31/07/2026 | Thêm gửi ảnh trong chat (đa ảnh, tự so sánh + tự đánh giá liên quan bài giảng), quiz theo từng trang (nút riêng mỗi slide, tự gộp trang liền kề khi thiếu nội dung), quiz thích ứng tự nhắm điểm yếu (mở rộng `interactionLog` với nhãn `image`/`quiz_wrong`, tái dùng `sortSlidesByInterest` có sẵn) | Học viên xin thêm 3 tính năng agent qua phản hồi thử nghiệm trực tiếp; ưu tiên nhóm tính năng agent trước UI polish |
| 31/07/2026 | Xoá `codebase/quiz-agent/` (Python CLI sketch) và `codebase/frontend/` (mock UI tĩnh) — cập nhật §8 và README | Cả 2 là artifact CP1-CP2 sớm, đã bị bản Working trong `codebase/src/`+`codebase/server/` vượt qua hoàn toàn về chức năng; giữ lại dễ gây hiểu nhầm đây là 1 phần prototype đang chấm (R5/R7) |
| 31/07/2026 | Thêm `eval/run_golden_set_live.mjs` (gọi thật Gemini qua backend, dùng chung golden_set.json + validator thật với bản offline) và chạy lượt đầu tiên: 15/17 (88,2%); regenerate lại `eval/eval_results.json` (offline) vì bản cũ 30/07 còn trích dẫn nội dung bài học đã bị thay (Day 2/Day 3, Trang 14/15/12/3/4/27/33) | `eval/eval_results.json` cũ tham chiếu trang/bài không còn tồn tại sau khi thay nội dung `data.js`, gây hiểu nhầm nếu bị hỏi tại CP5; đồng thời trước đó repo hoàn toàn chưa có công cụ chạy lượt live dù §8 đã phân công Sơn làm việc này |
| 31/07/2026 | Thêm 4 case mới (TC21-24) và gắn `source_turn_id`/`source_note` cho 10/24 case golden set, trỏ về turn thật trong `data/vlearn-pack/chatlog/` (mining lại bằng từ khoá chủ đề trùng với nội dung `data.js`); regenerate `eval/eval_results.json` (24/24 offline pass) | Rubric R4 yêu cầu ≥10 case golden set từ chatlog thật; 20 case gốc đều tự dựng theo slide, chưa case nào trỏ được về turn thật |
| 31/07/2026 | Đổi phân công §8: Canvas + `docs/agent-architecture-deck.html` (slide kiến trúc/agent, deploy trên Claude) chuyển từ Khôi sang Trần Đức Bảo Trung (2A202601269), thay cho `demo-slides.pdf`; cập nhật README | Trung là người thực sự làm Canvas và slide deck; mã HV của Trung trước đó còn để trống trong README |
